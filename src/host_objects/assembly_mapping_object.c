...
** sure that the following definition is set in abcc_object_config.h:
** EIP_OBJ_ENABLE              1
**
** If there is at least one write mappable assembly mapping instance defined,
** make sure that the following definitions are set in abcc_object_config.h
** EIP_IA_PROD_INSTANCE_ENABLE 1
**
...
static void InstanceCommand( ABP_MsgType* psNewMessage )
{
   ...
   case ABP_ASM_CMD_READ_ASSEMBLY_DATA:
   {
      ...
      if( ABCC_GetMaxMessageSize() < iMapSize )
      {
